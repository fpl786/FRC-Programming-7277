...
void DriveTrain::MainDrive(frc::Joystick* joystick, frc::XboxController* controller){
	...
	double yValue = 0;
	//if (controller -> GetY(GenericHID::JoystickHand::kLeftHand) == 0){
		yValue = controller -> GetY(GenericHID::JoystickHand::kLeftHand);
	//}else if(controller -> GetY(GenericHID::JoystickHand::kRightHand) == 0){
		//yValue = -(controller -> GetTriggerAxis(GenericHID::JoystickHand::kRightHand));
	//}
    driveTrain.ArcadeDrive(-yValue, controller->GetX(GenericHID::JoystickHand::kRightHand));
